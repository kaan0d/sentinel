...
| CI | GitHub Actions: Python 3.12 and 3.13 on Ubuntu and Windows, and the live capture tests on a real interface as root on Ubuntu (green) and as Administrator on Windows (stage 12: the first run failed one test of five, on a runner's loopback checksum, see Stage 12; the fix has not run yet) |
